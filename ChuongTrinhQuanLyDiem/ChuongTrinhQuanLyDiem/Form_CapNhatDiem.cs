...
{
    public partial class Form_CapNhatDiem : Form
    {
        public Form_CapNhatDiem()
        {
            InitializeComponent();
        }
...
//            dtgv.Columns[7].HeaderText = "Xếp loại";
        ...
        {
            dt.MonHoc_Insert(txtmaMon.Text, txtTenMon.Text, Convert.ToInt16(txtSoTC.Text), txtMaHK.Text);
        ...
        private void dtgv_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
               ...
               // double diem4 = Convert.ToDouble(Convert.ToDouble(dtgv.Rows[i].Cells[5].Value));
                dtgv.Rows[i].Cells["DiemTrenLop"].ReadOnly =true ;
        ...
        {
            //dtgv.DataSource = dt.BangDiemHP_2(cbLop.SelectedValue.ToString(), txtmaMon.Text);//
            cbLop_SelectedIndexChanged(sender, e);
        }
        ...
        {
            HuongDanNhapDiem a = new HuongDanNhapDiem();
            a.ShowDialog();
        }
        ...
        {
            int i = dtgv.CurrentCell.RowIndex;
            if(dtgv.Rows[i].Cells["DiemThi2"] == null)
            {
                MessageBox.Show("Sinh viên này đã thi lại, Bạn cần vào mục 'Quản Lý Thi Lại' để cập nhật!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            } 
        }
